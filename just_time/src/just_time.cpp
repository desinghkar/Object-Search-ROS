...
MapTimeProb::MapTimeProb(string inpath, string outpath, Point P, double theta)
{
	if(readExploredMap(inpath))
	{
		...
		for(int id=0; id<ORIENT; id++)
			cout<<"Time["<<id<<"] at 170, 170 is "<<Time[id].at<double>(200, 220)<<endl;
...
double MapTimeProb::timeProbability(int i, int j, int angle)
{
	double t1, t2;
	double v1 = 6, v2 = 1;
	double d1 = dist(current_location.x, current_location.y, i, j); //doubt in this stage
	double d2 = diff(current_orientation, 1.0*angle*M_PI/4);
	t1 = d1/v1;
	t2 = d2/v2;
	double result;
	if(t1+t2<1)
...
{
	MapTimeProb T(av[1], av[2], Point(170, 170), 0);
