...
class Potential_update
{
	...
	public:
	Potential_update(string path)
	{
		updatedPotentialMap =imread(path);
...
//		cout<<"Level 3 x="<<indexj<<" y="<<indexi<<endl;
		//Level 3-2
		indexi = updatedPotentialMap.rows/2 + indexi;
		indexj = updatedPotentialMap.cols/2 + indexj;
//		cout<<"Level 2 x="<<indexj<<" y="<<indexi<<endl;
		//Level 2-1
		indexi = updatedPotentialMap.cols - indexi;
...
*/	double convertQtoE(Mat& M)
	{
		...
		cout<<"Local_orientation is "<<(local_orientation*180/M_PI)<<endl;
	...
	int PatchTraceAt(int x, int y, double theta)
	{
		...
		cout<<"theta inside the PatchTrace is"<<theta<<endl;
...
int main(int argc, char * argv[])
{
	...
	if(argc<2)
	{
		cout<<"Pls enter the path of the original potential map"<<endl;
		return 0;
	}
	Potential_update P_up(argv[1]);
