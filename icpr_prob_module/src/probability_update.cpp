	...
	void updateProb()//const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& loc)
	{
		...
		while(1)
		{
			if(iter)
			{
				...
				cout<<"Read the image"<<image2_path<<endl;
...
//			multiplyProbTime();
	...
	{
		Mat fusedProbabilities(Prob1.size(), Prob1.type());
		Prob1.copyTo(fusedProbabilities);
		for(int i=0; i<Prob1.rows; i++)
			for(int j=0; j<Prob1.cols; j++)
			{
				
				float v1 = Prob1.at<float>(i, j);
				if(v1!=999 && v1!=888 && v1!=777 && v1!=555)
				{
				float v2 = Prob2.at<float>(i, j);
				float v3 = Prob3.at<float>(i, j);
				float v4 = Prob4.at<float>(i, j);
				float v5 = Prob5.at<float>(i, j);
				float v6 = Prob6.at<float>(i, j);
				float v7 = Prob7.at<float>(i, j);
				float v8 = Prob8.at<float>(i, j);
				float v = multNormalize(v1, v2);
				v = multNormalize(v, v3);
				v = multNormalize(v, v4);
				v = multNormalize(v, v5);
				v = multNormalize(v, v6);
				v = multNormalize(v, v7);
				v = multNormalize(v, v8);
				fusedProbabilities.at<float>(i, j) = v;
				}

			}
		disp_Probability(fusedProbabilities, 0, "fusedProbabilities.png", 0);
	}
	float multNormalize(float A, float B)
	{
		float N = A*B;
		float M = (1-A)*(1-B);
		return N/(N+M);
...
//		*theta = asin(2*(q0*q2 - q3*q1));
	...
	void getTableArea()
	{
		...
		if(TotalA<5)
		{
			...
			cout<<"Map image saved for verification of the objects are not available"<<endl;
			imwrite("exploredmap.png", P_map);
			exit(0);
		}
	}
...
//		disp_Probability(Time, theta/45, path);
	...
	void updateVPProb(Mat & Prob, float theta, string path)
	{
		...
		string image_name = path+"buff2_map.png";
		imwrite(image_name, buff2_map);
...
//		kcount = 0;
		...
		for(float si=min_theta; si<max_theta+1; si++)
		{
...
//			return 0;
		...
		while(++vector <=steps)
		{
			...
			if(B==0&&G==255&&R==0)
			{
				...
				buff2_map.at<Vec3b>(ky, kx)[0] = 0; 
				buff2_map.at<Vec3b>(ky, kx)[1] = 0;
